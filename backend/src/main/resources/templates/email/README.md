...
Declare the Thymeleaf namespace on the enclosing `html` element. Pass localized text to header/footer; callers may use different context names, as [quote-session.html](quote-session.html) does. The context must also supply `logoUrl` (the absolute configured frontend URL plus `/assets/images/SVG/logo-giallo-spesso.svg`) and `currentYear`.
