...
describe('SessionEmailComponent', () => {
  it('copies the saved server link without needing an email address', fakeAsync(() => {
    const estimator = jasmine.createSpyObj<QuoteEstimatorService>('estimator', ['sessionLink']);
    const link = 'https://example.test/it/calculator/basic?session=test';
    estimator.sessionLink.and.returnValue(of({ url: link }));
    const clipboard = spyOn(navigator.clipboard, 'writeText').and.returnValue(Promise.resolve());
    TestBed.configureTestingModule({
      imports: [SessionEmailComponent, TranslateModule.forRoot()],
      providers: [
        { provide: QuoteEstimatorService, useValue: estimator },
        { provide: OrderInformationService, useValue: { busy: signal(false), model: signal('') } },
      ],
    });
    ...
    buttons.find(button => button.textContent?.trim() === 'SESSION_EMAIL.COPY')!.click();
    flushMicrotasks(); fixture.detectChanges();
    expect(clipboard).toHaveBeenCalledWith(link);
    expect(fixture.componentInstance.form.invalid).toBeTrue();
    expect(fixture.nativeElement.querySelector('[role=status]').textContent).toContain('SESSION_EMAIL.COPIED');
    fixture.destroy();
  }));

  it('requires an updated quote and prevents duplicate sends until completion', () => {
    const response = new Subject<void>();
    const estimator = jasmine.createSpyObj<QuoteEstimatorService>('estimator', ['emailSession']);
    ...
    fixture.componentInstance.form.controls.email.setValue('customer@example.test');
    ...
    expect(fixture.nativeElement.querySelector('[role=status]').textContent).toContain('SESSION_EMAIL.SENT');
