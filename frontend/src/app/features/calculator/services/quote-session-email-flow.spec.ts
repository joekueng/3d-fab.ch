...
describe('Session email and cross-device restore', () => {
  ...
  const saved = { id: 'email-draft', entries: [{
    id: 'entry', text: 'Print upright', model: '', modelKey: '', createdAt: '', readAt: null,
    attachments: [{ id: 'file', name: 'drawing.pdf', mime: 'application/pdf', size: 12 }],
  }] };
  ...
  it('restores notes and server attachments using only the emailed URL', fakeAsync(() => {
    ...
    let restored = false;
    estimator.getQuoteSession('email-session').subscribe(() => restored = true);
    ...
    http.expectOne(api + '/quote-sessions/email-session').flush({
      session: { informationDraftId: 'email-draft' }, items: [],
    });
    const draft = http.expectOne(api + '/information-drafts/email-draft');
    expect(draft.request.headers.get('X-Information-Token')).toBe('link-secret');
    draft.flush(saved); flushMicrotasks();
    ...
    const download = http.expectOne(api + '/information-drafts/email-draft/files/file');
    expect(download.request.headers.get('X-Information-Token')).toBe('link-secret');
    download.flush(new Blob(['%PDF-drawing'])); flushMicrotasks();
  }));

  it('saves and scans pending files before requesting the email', fakeAsync(() => {
    information.text.set('Print upright');
    information.files.set([new File(['%PDF-drawing'], 'drawing.pdf', { type: 'application/pdf' })]);
    let sent = false;
    estimator.emailSession('email-session', 'customer@example.test', 'it', 'easy').subscribe(() => sent = true);
    http.expectOne(api + '/information-drafts').flush({ id: 'email-draft', token: 'link-secret' }); flushMicrotasks();
    ...
    upload.flush(saved); flushMicrotasks();
    const email = http.expectOne(api + '/quote-sessions/email-session/email');
    expect(email.request.body.information).toEqual({ id: 'email-draft', token: 'link-secret' });
    expect(sent).toBeFalse();
    email.flush(null); flushMicrotasks();
  ...
  it('does not email when the scan fails and retains pending work', fakeAsync(() => {
    ...
    information.files.set([new File(['%PDF-drawing'], 'drawing.pdf', { type: 'application/pdf' })]);
    let failed = false;
    estimator.emailSession('email-session', 'customer@example.test', 'it', 'easy').subscribe({ error: () => failed = true });
    http.expectOne(api + '/information-drafts').flush({ id: 'email-draft', token: 'link-secret' }); flushMicrotasks();
    http.expectOne(api + '/information-drafts/email-draft').flush({}, { status: 503, statusText: 'Unavailable' }); flushMicrotasks();
    http.expectNone(api + '/quote-sessions/email-session/email');
  ...
  it('does not load session data when the emailed link has expired', () => {
    window.history.replaceState(null, '', '?session=email-session');
    let failed = false;
    estimator.getQuoteSession('email-session').subscribe({ error: () => failed = true });
    http.expectOne(api + '/quote-sessions/email-session/resume').flush({}, { status: 410, statusText: 'Gone' });
  ...
  it('links newly saved information to a legacy session before publishing its complete URL', fakeAsync(() => {
    ...
    http.expectOne(api + '/information-drafts').flush({ id: 'email-draft', token: 'link-secret' }); flushMicrotasks();
    http.expectOne(api + '/information-drafts/email-draft').flush(saved); flushMicrotasks();
    expect(window.location.hash).toBe('');
    const association = http.expectOne(api + '/quote-sessions/email-session/information');
    expect(association.request.body).toEqual({ id: 'email-draft', token: 'link-secret' });
    association.flush(null); flushMicrotasks();
    expect(window.location.hash).toBe('');
    expect(information.attachments().length).toBe(1);
  }));
});
