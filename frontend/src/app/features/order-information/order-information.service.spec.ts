...
describe('OrderInformationService', () => {
  ...
  const attachment = { id: 'file', name: 'photo.png', size: 20, mime: 'image/png' };
  const stored = { id: 'draft', entries: [{ id: 'entry', text: 'Vertical', model: 'part.stl', modelKey: 'part-key', createdAt: '2026-09-11T10:00:00Z', readAt: null, attachments: [attachment] }] };
  beforeEach(() => {
    TestBed.configureTestingModule({ providers: [provideHttpClient(), provideHttpClientTesting()] });
    service = TestBed.inject(OrderInformationService); http = TestBed.inject(HttpTestingController);
  });
  afterEach(() => { http.verify(); localStorage.removeItem('information-draft:draft'); localStorage.removeItem('order-information:order'); });

  it('keeps the same draft credential while saving and recalculating', fakeAsync(() => {
    service.text.set('Vertical'); service.model.set('part-key'); service.modelName.set('part.stl');
    ...
    expect(service.files()).toEqual([]); expect(service.attachments()).toEqual([attachment]);
    ...
    save.flush(stored); flushMicrotasks();
  ...
  it('retains input after failed upload and never silently replaces an inaccessible draft', fakeAsync(() => {
    service.text.set('Keep me'); const file = new File(['pdf'], 'note.pdf', { type: 'application/pdf' }); service.files.set([file]);
    void service.saveDraft().catch(() => undefined);
    http.expectOne(api + '/information-drafts').flush({ id: 'draft', token: 'secret' }); flushMicrotasks();
    http.expectOne(api + '/information-drafts/draft').flush({}, { status: 400, statusText: 'Bad request' }); flushMicrotasks();
    expect(service.text()).toBe('Keep me'); expect(service.files()).toEqual([file]); expect(service.error()).toBeTrue();
    void service.useDraft('foreign-draft'); flushMicrotasks();
    void service.saveDraft().catch(() => undefined); flushMicrotasks();
    http.expectNone(api + '/information-drafts'); expect(service.error()).toBeTrue();
  }));
  ...
  it('sends separate order credentials and admin session cookies to their respective endpoints', fakeAsync(() => {
    ...
    expect(customer.request.headers.get('X-Information-Token')).toBe('order-secret'); customer.flush({ id: 'order', entries: [] });
    void service.markRead('order', ['known-entry']);
    const admin = http.expectOne(api + '/admin/orders/order/information/read');
    expect(admin.request.withCredentials).toBeTrue(); expect(admin.request.body).toEqual({ entries: ['known-entry'] });
    expect(admin.request.headers.has('X-Information-Token')).toBeFalse(); admin.flush({ id: 'order', entries: [] }); flushMicrotasks();
  }));
});
