...
export interface InformationCredential {
  id: string;
  token: string;
}
...
export class OrderInformationService {
  ...
  private write(key: string, value: string): void {
    try {
      ...
      /* Current tab keeps its credential. */
    }
  }
  rememberOrder(id: string, token: string): void {
    this.orderTokens.set(id, token);
    this.write('order-information:' + id, token);
  }
  orderToken(id: string): string {
    return (
  ...
  async useDraft(
    ...
    if (id && this.credential?.id === id && !this.inaccessible) {
    ...
    const loading = (async () => {
      ...
      try {
        const value = await firstValueFrom(
          this.http.get<Information>(`${this.api}/information-drafts/${id}`, {
            headers: this.headers(token),
          }),
        );
        if (generation === this.generation) {
          this.applyDraft(value);
          this.write('information-draft:' + id, token);
          this.publishLink();
          this.error.set(false);
        }
      } catch {
  ...
  private applyDraft(value: Information): void {
    this.text.set(value.entries[0]?.text || '');
    this.model.set(value.entries[0]?.modelKey || '');
    this.modelName.set(value.entries[0]?.model || '');
    this.attachments.set(value.entries.flatMap((e) => e.attachments));
    this.files.set([]);
  }
  ...
  private async persistDraft(): Promise<InformationCredential> {
    ...
    try {
      ...
      if (!credential) {
        credential = await firstValueFrom(
          this.http.post<InformationCredential>(
            `${this.api}/information-drafts`,
            {},
          ),
        );
      ...
      const value = await firstValueFrom(
        this.http.put<Information>(
          `${this.api}/information-drafts/${credential.id}`,
          body,
      ...
      if (this.model() === snapshot.model)
        this.model.set(entry?.modelKey || '');
      if (this.modelName() === snapshot.modelName)
        this.modelName.set(entry?.model || '');
        ...
        value.entries
          ...
          .filter((file) =>
            existingIds.has(file.id)
              ? remainingIds.has(file.id)
              : pendingFiles.has(snapshot.files[uploadedIndex++]),
          ),
      );
      this.files.update((files) =>
        files.filter((file) => !snapshot.files.includes(file)),
      );
      if (this.quoteId) {
        await firstValueFrom(
          this.http.put<void>(
            `${this.api}/quote-sessions/${this.quoteId}/information`,
            credential,
          ),
        );
        if (generation !== this.generation) throw new Error('Draft changed');
  ...
  getOrder(id: string, admin: boolean): Promise<Information> {
    return firstValueFrom(
      this.http.get<Information>(
        this.orderUrl(id, admin),
        this.options(id, admin),
      ),
    );
  }
  append(
    ...
    return firstValueFrom(
      this.http.post<Information>(
        this.orderUrl(id, false),
        this.form(text, model, modelName, files, []),
  ...
  unread(): Promise<string[]> {
    return firstValueFrom(
      this.http.get<string[]>(`${this.api}/admin/orders/information-unread`, {
        withCredentials: true,
      }),
    );
  }
  async blob(
    ...
    const base = orderId
      ? this.orderUrl(orderId, admin)
      : `${this.api}/information-drafts/${this.credential?.id}`;
    const options = orderId
      ? this.options(orderId, admin)
      : { headers: this.headers(this.credential?.token || '') };
    return firstValueFrom(
      this.http.get(`${base}/files/${file.id}`, {
        ...options,
        responseType: 'blob',
      }),
    );
  }
  private orderUrl(id: string, admin: boolean): string {
    return `${this.api}/${admin ? 'admin/' : ''}orders/${id}/information`;
  }
  private options(
    id: string,
    admin: boolean,
    ...
    return {
      headers: admin ? {} : this.headers(this.orderToken(id)),
      withCredentials: admin,
    };
  }
  private headers(token: string): Record<string, string> {
    return { 'X-Information-Token': token };
  }
  private form(
    text: string,
    model: string,
