...
describe('OrderInformationComponent', () => {
  ...
  afterEach(() => {
    fixture.destroy();
    http.verify();
    localStorage.removeItem('order-information:test-order');
  });
  ...
  it('edits notes, previews photos, removes selected files and keeps the collapsed summary', fakeAsync(() => {
    ...
    button('Aggiungi istruzioni o allegati').click();
    fixture.detectChanges();
    tick();
    const textarea: HTMLTextAreaElement =
      fixture.nativeElement.querySelector('textarea');
    textarea.value = 'Stampare in verticale';
    ...
    transfer.items.add(new File(['photo'], 'photo.png', { type: 'image/png' }));
  ...
  it('shows draft instructions in checkout and allows editing without losing them', fakeAsync(() => {
    service.text.set('Conservare la superficie esterna');
    fixture.componentRef.setInput('review', true);
    fixture.detectChanges();
    tick();
    expect(fixture.nativeElement.textContent).toContain(
      'Conservare la superficie esterna',
    );
    button('Modifica').click();
    fixture.detectChanges();
    tick();
    fixture.detectChanges();
    expect(
      (fixture.nativeElement.querySelector('textarea') as HTMLTextAreaElement)
        .value,
    ).toBe('Conservare la superficie esterna');
  }));
  it('adds information after purchase and retains the earlier entry in the rendered history', fakeAsync(() => {
    service.rememberOrder('test-order', 'secret');
    fixture.componentRef.setInput('orderId', 'test-order');
    fixture.detectChanges();
    const first = {
      id: 'first',
      text: 'Originale',
    ...
    http
      .expectOne(api + '/orders/test-order/information')
      .flush({ id: 'info', entries: [first] });
    flushMicrotasks();
    fixture.detectChanges();
    button('Aggiungi informazioni').click();
    fixture.detectChanges();
    tick();
    const textarea: HTMLTextAreaElement =
      fixture.nativeElement.querySelector('textarea');
    textarea.value = 'Seconda indicazione';
    textarea.dispatchEvent(new Event('input'));
    fixture.detectChanges();
    tick();
    expect(fixture.nativeElement.textContent).toContain(
      'se la stampa è già iniziata',
    );
    button('Salva').click();
    const save = http.expectOne(api + '/orders/test-order/information');
    expect(save.request.method).toBe('POST');
    save.flush({
      id: 'info',
      entries: [first, { ...first, id: 'second', text: 'Seconda indicazione' }],
    });
    flushMicrotasks();
    fixture.detectChanges();
    expect(fixture.nativeElement.textContent).toContain('Originale');
    expect(fixture.nativeElement.textContent).toContain('Seconda indicazione');
    expect(fixture.nativeElement.textContent).toContain(
      'Informazioni aggiunte all’ordine',
    );
  }));
  it('acknowledges only the entries currently shown to the administrator', fakeAsync(() => {
    ...
    http
      .expectOne(api + '/admin/orders/test-order/information')
      .flush({ id: 'info', entries: [entry], customerToken: 'secret' });
    flushMicrotasks();
    fixture.detectChanges();
    expect(fixture.nativeElement.textContent).toContain('Nuove informazioni');
    button('Segna come lette').click();
    const read = http.expectOne(
      api + '/admin/orders/test-order/information/read',
    );
    expect(read.request.body).toEqual({ entries: ['entry'] });
    read.flush({
      id: 'info',
      entries: [{ ...entry, readAt: '2026-09-11T13:00:00Z' }],
    });
    flushMicrotasks();
    fixture.detectChanges();
    ...
    expect(fixture.componentInstance.customerLink()).toContain(
  ...
  it('keeps long filenames within the card at narrow widths', fakeAsync(() => {
    ...
    service.files.set([
      new File(['pdf'], 'a'.repeat(180) + '.pdf', { type: 'application/pdf' }),
    ]);
    fixture.detectChanges();
    button('Modifica').click();
    fixture.detectChanges();
    ...
    const attachments: HTMLElement =
      fixture.nativeElement.querySelector('.attachments');
    expect(attachments.scrollWidth).toBeLessThanOrEqual(
      attachments.clientWidth + 1,
    );
    expect(
      (fixture.nativeElement as HTMLElement).scrollWidth,
