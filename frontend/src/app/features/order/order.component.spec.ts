...
describe('OrderComponent payment tracking', () => {
  ...
  beforeEach(() => {
    api = jasmine.createSpyObj<QuoteEstimatorService>('api', ['getOrder', 'reportPayment']);
  ...
  it('distinguishes paid, production, completed and cancelled', () => {
    ...
    expect(component.trackingStep({ ...paid, status: 'IN_PRODUCTION' })).toBe(3);
  ...
  it('continues polling after reporting and slows down after confirmation', fakeAsync(() => {
    api.getOrder.and.returnValue(of({ id: 'fixture', status: 'PENDING_PAYMENT', paymentStatus: 'REPORTED' }));
    component.loadOrder();
    tick(10_000);
  ...
  it('ignores a GET response started before the report mutation', () => {
    ...
    stale.next({ ...paid, status: 'PENDING_PAYMENT', paymentStatus: 'PENDING' });
    stale.complete();
    expect(component.order()?.status).toBe('PAID');
  });
  ...
  it('keeps displayed data during a temporary network error', () => {
    ...
    const request = new Subject<typeof paid>(); api.getOrder.and.returnValue(request);
    component.loadOrder(); request.error(new Error('offline'));
  ...
  it('stops timers after destruction', fakeAsync(() => {
    api.getOrder.and.returnValue(of(paid));
    component.loadOrder(); component.ngOnDestroy(); tick(120_000);
    expect(api.getOrder).toHaveBeenCalledTimes(1);
  }));
});

@Component({ selector: 'app-order-information', standalone: true, template: '' })
...
describe('OrderComponent rendered timeline', () => {
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: Router, useValue: {} },
        { provide: OrderInformationService, useValue: { resumeOrder: () => Promise.resolve() } },
        { provide: QuoteEstimatorService, useValue: { getOrder: () => of({ id: 'fixture', status: 'PAID', paymentStatus: 'RECEIVED' }), getTwintPayment: () => of({}) } },
      ],
    });
    TestBed.overrideComponent(OrderComponent, {
      remove: { imports: [OrderInformationComponent] }, add: { imports: [InformationStub] },
    });
    const translations = TestBed.inject(TranslateService);
    translations.setTranslation('it', { TRACKING: {
      STEP_PENDING: 'In attesa di pagamento', STEP_REPORTED: 'In verifica', STEP_PAID: 'Pagato',
      STEP_PRODUCTION: 'In produzione', STEP_SHIPPED: 'Spedito',
      PAID_DESCRIPTION: 'Pagamento confermato. In attesa di produzione.', STATUS_CANCELLED: 'Ordine annullato',
    } });
    translations.use('it');
  });
  ...
  it('renders five steps with paid active, then production, and no progress for cancellation', fakeAsync(() => {
    ...
    expect(root.querySelector('.timeline-step.active')?.textContent).toContain('Pagato');
    expect(root.textContent).toContain('Pagamento confermato. In attesa di produzione.');
    fixture.componentInstance.order.update(order => ({ ...order!, status: 'IN_PRODUCTION' }));
    fixture.detectChanges();
    expect(root.querySelector('.timeline-step.active')?.textContent).toContain('In produzione');
    fixture.componentInstance.order.update(order => ({ ...order!, status: 'CANCELLED' }));
    fixture.detectChanges();
